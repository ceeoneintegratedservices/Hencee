...
export default function Sidebar({ currentPage = "dashboard", sidebarOpen = true, setSidebarOpen }: SidebarProps) {
  ...
  const menuItems = isInitialized ? getMenuItems() : [];
        ...
        <nav className="flex flex-col gap-4 mt-6 px-6 flex-1 overflow-y-auto">
          {menuItems.map((item) => {
            ...
            const hrefMap: { [key: string]: string } = {
              ...
              'sales': '/orders',
              'customers': '/customers',
              'products': '/inventory', // Map products to inventory since there's no dedicated products page
              'inventory': '/inventory',
              'reports': '/reports',
              'users': '/users-roles',
              'settings': '/settings',
            ...
            const href = hrefMap[item.key] || `/${item.key}`;
